...
Then, install vcpkg and set environment variable by follwing these steps:
 Open a terminal, then run the following commands from your initial directory (excluding the >):
...
Windows (CMD):
3a. > bootstrap-vcpkg.bat

Windows (Powershell):
3b. > .\bootstrap-vcpkg.bat

Linux/MacOS (Bash or Zsh):
3c. > ./bootstrap-vcpkg.sh
...
Windows (CMD):
4a. > setx VCPKG_ROOT "%cd%"
5a. > setx PATH "%cd%;%PATH%"

Windows (Powershell):
4a. > setx VCPKG_ROOT "$pwd"
5a. > setx PATH "$pwd;$env:PATH"

Linux/MacOS (Bash)
4a. > echo "export VCPKG_ROOT=$PWD" >> ~/.bash_profile
5a. > echo "export PATH=\"$PWD:$PATH\"" >> ~/.bash_profile

Linux/MacOS (Zsh)
4a. > echo "export VCPKG_ROOT=$PWD" >> ~/.zshenv
5a. > echo "export PATH=\"$PWD:$PATH\"" >> ~/.zshenv
